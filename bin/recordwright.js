...
function getPidPath() {
    return path.join(__dirname, 'recordwright.pid')
}
    ...
    switch (cli.command) {
        ...
        case 'start':
            ...
            if (fsSync.existsSync(cli.args.path)) {
                ...
                process.env.bluestonePath = path.join(path.resolve(cli.args.path), 'recordwright.json')
            }
            else {
            ...
            let configPath = config.code.configPath
