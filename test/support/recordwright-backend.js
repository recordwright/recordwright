...
class RecordWrightBackEnd {
    constructor() {
        this.port = testConfig.app.port
    ...
    /**
     * based on the file name, get the resource
     * @param {string} fileName 
     */
    async getJsResource(fileName) {
        let res = await axios.get(`${this.url}/resource/js/${fileName}`)
        return res
    }
    async waitInitComplete() {
        let res = await axios.get(`${this.url}/resource/api/`)
    }
    async getMainPage() {
...
module.exports = RecordWrightBackEnd