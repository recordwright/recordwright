...
const options = {
    path: {
        description: 'The path to the folder where we can find recordwright.json',
        type: 'string'
    },
    initPath: {
        description: 'The path to the folder where we can initialize recordwright automation project',
        type: 'string'
    },
    url: {
    ...
    port: {
        default: '3600',
        description: 'the port for recordwright backend server',
        nargs: 1,
        type: 'string',
        global: false
    },
    pidfile: {
        default: 'recordwright.pid',
...
let argv = yargs
    .usage('Usage: recordwright [command=start] [options...]')
    .command('start <path>', 'Starts the server based on the project specified', startYargs => {
        startYargs
            .usage('Usage: recordwright start <path>')
            .help('help')
            .wrap(null)
            .positional('path', options.path)
            .options({ port: options.port })
            .example('recordwright start ./sample_project --port 3600',
                'Starts on port 3600')
    })
    .command('edit <tcId>', 'Edit Testcase', recordYargs => {
        recordYargs
            ...
            .wrap(null)
            .example('recordwright edit testcaseName --result ./result.json', 'Edit testcaseName based on result from ./result.json')
    })
    .command('init <path>', 'Initialize recordwright Automation Project', recordYargs => {
        recordYargs
            .usage('Usage: recordwright init')
            .help('help')
            .positional('path', options.initPath)
            .wrap(null)
            .example('recordwright init .', 'Create a recordwright Autoamtion Project in current folder')

    })
    .command('record <url>', 'Record new workflow', recordYargs => {
        recordYargs
            .usage('Usage: recordwright record <url>')
            .help('help')
            .positional('url', options.url)
            .wrap(null)
            .example('recordwright record https://www.google.com', 'start record script against https://www.google.com ')

    })
    .command('function <function relative folder> <function name>', 'Create function template', funcYargs => {
        funcYargs
            .usage('Usage: recordwright function <folder> <name>')
            .help('help')
            .wrap(null)
            .positional('folder', options['function relative folder'])
            .positional('name', options['function name'])
            .example('recordwright function ./abc testfunction',
                'creaet a testfunction under ./function/abc/testFunction.js')
    })
    .command('compile', 'Do hot reload on customized function', funcYargs => {
        funcYargs
            .usage('Usage: recordwright compile')
            .help('help')
            .wrap(null)
            .example('recordwright compile',
